...
export class GiphyEffects {
  constructor(private actions$: Actions, private giphyService: GiphyService, private store: Store<AppState>, private router: Router) {}

  trending$ = createEffect(() =>
    this.actions$.pipe(
      ofType(trending),
      mergeMap(() => {
        return this.giphyService.trending().pipe(
          map(giphies => {
  ...
  search$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(action => {
        return this.giphyService.search(action.search).pipe(
  ...
  loadMore$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(([action, urlParams]: [Action, { search: string; page: number }]) => {
        return this.giphyService.search(urlParams.search, urlParams.page * OFFSET).pipe(
  ...
  loadOne$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(([action, giphyId]: [Action, string]) => {
        return this.giphyService.loadOne(giphyId).pipe(
